...
const getRipgrepConfig = memoize((): RipgrepConfig => {
  const getSystemConfig = (): RipgrepConfig | null => {
    const { cmd: systemPath } = findExecutable('rg', [])
    if (systemPath === 'rg') {
      return null
    }
    // SECURITY: Use command name 'rg' instead of systemPath to prevent PATH hijacking.
    // Let the OS resolve the binary instead of executing a discovered path directly.
  ...
  if (systemConfig) {
    logForDebugging(
      `Vendored ripgrep missing at ${command}; falling back to system rg`,
    )
    return systemConfig
  }
